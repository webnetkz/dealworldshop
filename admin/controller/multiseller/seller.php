...
class ControllerMultisellerSeller extends ControllerMultisellerBase {
		...
		foreach ($results as $result) {
			...
			$columns[] = array_merge(
				...
				array(
					...
					'seller' => "<a href='".$this->url->link((version_compare(VERSION, '2.1.0.0', '<') ? 'sale' : 'customer').'/customer/edit', 'token=' . $this->session->data['token'] . '&customer_id=' . $result['seller_id'], 'SSL')."'>{$result['c.name']}({$result['ms.nickname']})</a>",
	...
	public function jxSaveSellerInfo() {
		...
		$json = array();
		$this->load->model((version_compare(VERSION, '2.1.0.0', '<') ? 'sale' : 'customer').'/customer');
		...
		if (empty($data['seller']['seller_id'])) {
			...
			if (empty($data['customer']['customer_id'])) {
				...
				$this->language->load((version_compare(VERSION, '2.1.0.0', '<') ? 'sale' : 'customer').'/customer');
