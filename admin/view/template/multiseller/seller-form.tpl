  ...
  <div class="container-fluid">
        ...
        <form id="ms-sellerinfo" class="form-horizontal">
            ...
            <legend><?php echo $ms_catalog_sellerinfo_customer_data; ?></legend>
             <div class="form-group required">
                ...
                <?php } else { ?>
                    <a href="<?php echo $this->url->link((version_compare(VERSION, '2.1.0.0', '<') ? 'sale' : 'customer').'/customer/edit', 'token=' . $this->session->data['token'] . '&customer_id=' . $seller['seller_id'], 'SSL'); ?>"><?php echo $seller['name']; ?></a>
                <?php } ?>
                </div>
            </div>
	...
	$(function() {
		...
		$("select[name='seller[country]']").bind('change', function() {
			$.ajax({
				url: "index.php?route=<?php echo (version_compare(VERSION, '2.1.0.0', '<') ? 'sale' : 'customer'); ?>/customer/country&token=<?php echo $token; ?>&country_id=" + this.value,
				dataType: 'json',
				beforeSend: function() {
