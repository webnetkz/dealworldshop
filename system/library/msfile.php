...
class MsFile extends Model {
	...
	private function myresize($file)	{
		$tmp_path = DIR_IMAGE . $this->config->get('msconf_temp_image_path');
		$translitname = $this->rename($file['name']);
		$filename = time() . '_' . md5(rand()) . '.'.$translitname;
		...
		if ($w_src > $w)
		{
			...
			imagejpeg($dest, $tmp_path . $filename, 70);
			imagedestroy($dest);
			imagedestroy($src);
		...
		{
			// Вывод картинки и очистка памяти
			imagejpeg($src, $tmp_path . $filename, 70);
			imagedestroy($src);
	...
	private function translitName($string) { 
		$replace = array(
		...
		$str = preg_replace('![^\w\d\s\-]*!','',$str); // Убрать все левые символы из строки		
		$str = preg_replace('/\s+/', '-', $str); // Убрать двойные пробелы
		$str = preg_replace('| |','-',$str); // Заменить одинарные пробелы на тире
		$str = preg_replace('|-+|','-',$str); // Заменить повторяющиеся тире на 1
