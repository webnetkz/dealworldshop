...
class ControllerSellerAccountProduct extends ControllerSellerAccount {
	public function getTableData() {
		...
		foreach ($products as $product) {
			...
			$product['p.price'] = (int)$product['p.price'];
			if ($special) {
	...
	public function jxSubmitProduct()
	{
		...
		foreach ($data['languages'] as $language_id => $language) {
			...
			if (in_array('metaTitle', $this->config->get('msconf_product_included_fields'))) {
				$data['languages'][$language_id]['product_meta_title'] = $data['languages'][$default]['product_meta_title'];
			} else {
				$data['languages'][$language_id]['product_meta_title'] = $data['languages'][$default]['product_name'];
			}
			if (in_array('metaDescription', $this->config->get('msconf_product_included_fields'))) {
				...
				$data['languages'][$language_id]['product_meta_description'] = mb_substr(strip_tags(htmlspecialchars_decode($data['languages'][$language_id]['product_description'], ENT_COMPAT)),0,249);
			}
			if (in_array('metaKeywords', $this->config->get('msconf_product_included_fields'))) {
				...
				$data['languages'][$language_id]['product_meta_keyword'] = $data['languages'][$default]['product_name'];
			}
		...
		*/
		...
		if ($this->config->get('msconf_enable_seo_urls_product')) {
			...
			if ($this->config->get('msconf_enable_non_alphanumeric_seo') && $non_latin_chars) {
				$data['keyword'] = $this->translit_url($data['languages'][$default]['product_name']);
	...
	private function translit_url($string){
		 $replace = array(
